...
// 생성자 조건
// - 생성자 이름은 클래스 이름과 동일
// - 리턴값이 없는 메서드 형식 (void 타입 생략)(반환X)

// '컴파일러가 제공하는 기본 생성자'
// : 클래스에 생성자가 없으면 컴파일러가 자동으로 생성해줌(작성 시 이전 자동생성 클래스는 사라짐)
...
class Car {
    ...
    Car(String color) {
        ...
        // 해당 생성자가 수행하는 작업ㅇ르 유지하기 위함(무의미함 방지).
