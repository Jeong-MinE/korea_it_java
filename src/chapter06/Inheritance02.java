...
class Mammal {
    ...
    Mammal(String name) {
        // 필드와 지역변수의 이름이 같을 경우 충돌 방지를 위해 필드에 this. 키워드로 구분
...
class Cat extends Mammal {
    ...
    // Cat {
    // super(); - 상속을 받는 클래스의 기본 생성지에 반드시 포함!
    // }
...
//        System.out.println("자식 이름: " + name); 가능

        // 부모 클래스가 가진 인스턴스 변수에 접근
        // : super(생략불가능)
        System.out.println("부모 이름: " + super.name);
//        System.out.println("부모 이름: " + name); 불가능(자식 이름 name있을 경우 자식 이름 name으로 인식)
    }
}
...
public class Inheritance02 {
    public static void main(String[] args) {
        ...
        //      - 부모 크래스 내의 필드와 메서드에 접근. 연산자를 사용
        //      super.필드명/super.메서드명()
        // super() : 부모 클래스 내에서 생성자 호룰
