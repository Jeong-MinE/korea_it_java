package chapter08;
    ...
    // 1) 클래스 자체로 인스턴스 생성 X
    // 2) 하나 이상의 추상 메서드를 포함
    // 3) 확장성 제공을 위해 사용 - 자식 클래스들이 공통된 기능을 공유 + 고유 기능을 구현하도록 강제
    // 4) 다중 상속 불가능! - 하나의 extends 만 가능

    // +) 메서드 오버라이딩과 추상 메서드
    // - 추상 메서드 : 반드시 자식 클래스에서 오버라이딩해야함
    ...
    // class Liger extends Lion extends Tiger { // Error 다중 상속 불가능
//        void mane()
//    }
...
//        }
    ...
    // >> 자식 클래스에서 해당 메서드를 반드시 구현하도록 강제하는 역할 담당!
    // >> 'abstract' 키워드를 사용 + 구현부 '{}' 대신 세미콜론 ';' 을 사용하여 선언을 마침

    // cf) 메서드의 구성 : 선언부 + 구현부
    // (접근제어자←생략가능) 반환타입 메서드명 (매개변수...) {메서드의 기능 동작)
