...
// === ArrayList VS LinkedList === //
    ...
    // - 순차적으로 데이터를 관리 : 빠른 데이터 조회 적합
    // - 중간 삽입/삭제 시 : 데이터 이동이 필요하므로 상대적으로 성능이 저하

    // EX) 정렬된 책장
    // 책의 번호를아는 경우 해당 책장에서 책을 빠르게 가져올 수 있음
    // 새 책을 책장의 중간에 추가할 경우 - 책을 모두 한 칸씩 옳겨야 함(시간이 오래걸림)

    // 2) LinkedList
    // : 내부적으로 이중 연결 리스트를 사용하여 요소르 저장
    // : 빠른 삽입/삭제 성능, 느린 데이터 접근 성능
    // - 연결 기반 데이터 관리 : 비순차적 데이터 작업에 적합
    // - 특정 요소 접근 시 : 순차 탐색이 필요하므로 성능 저하

    // EX) 도서 대출 리스트
    // : 새로운 대출 기록을 추가하거나 기존 기록을 제거하는 것이 용이함
...
public class B_List {
    public static void main(String[] args) {
        ...
        //307574700ns
        System.out.println("ArrayList 삽입 기간 : " + (endTime - startTime) + "ns");

        LinkedList<Integer> linkedListTest = new LinkedList<>();
        startTime = System.nanoTime();
        for (int i = 0; i < 100000; i++) {
            linkedListTest.add(0, i); // 항상 맨 앞에 삽임
        }
        endTime = System.nanoTime();

        // 2253100ns - 약 200만 나노초
        System.out.println("LinkedList 삽입 기간 : " + (endTime - startTime) + "ns");
    }
}
