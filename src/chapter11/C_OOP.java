...
abstract class PaypalPaymentProcessor implements PaymentProcessor {
    @Override
    public void processPayment(Order order) {
...
class OrderService {
    ...
    // cf) CreditCardPaymentProcessor, PaypalPaymentProcessor(저수준 모델)
    ...
    public OrderService(PaymentProcessor paymentProcessor, GenerateReport generateReport) {
        this.paymentProcessor = paymentProcessor; // 결제 처리가 초기화
        this.generateReport = generateReport; // 보고서 생성기 초기화
    }

    // 주문 처리 메서드
    void placeOrder(Order order) {
        paymentProcessor.processPayment(order); // 결제 처리기에 결제 로직을 호출
        generateReport.generateReport(order); // 보고서 생성기에 보고서 생성 로직호출
