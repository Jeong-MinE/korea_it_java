...
// >> 낮은 결합도
//      : 프로그램 유지 보수하기 쉽게 만들고, 새로운 기능을 추가하거나 기존 코드를 수정할 때 오류 발생 가능성을 낮춤.

// 2. 응집도(높은 응집도)
// : 하나의 클래스나 모듈 안에서 관련된 작업들끼리 얼마나 잘 모여 있는지를 타나냄
...
//      >> 구현이 변겨되거나 확장되어도 인터페이스에만 맞추면 동작
...
class User {
    // 2-2 캡슐화
    //      : 필드와 private 설정, getter와 setter를 통해 접근
    //      : 클래스 내부 동작을 숨기고, 외부와의 상호작용을 최소화
    private String name;
...
class UserService {
    ...
    public void registerUser(User user) {
        ...
        if (user.getName().isEmpty()) {
            System.out.println("이름은 필수 사항 입니다.");
            return;
        }
...
// 높은 응집도 : 단일 책입 원칙 준수, 캡슐화, 모듈화(메서드 분리)