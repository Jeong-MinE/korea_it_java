...
* : 클래스 간의 관계를 명확하게 함 - 내부 클래스는 외부 클래스 멤버(필드, 메서드)에 접근 가능
*
* == 종류 ==
...
class OuterClass {
    ...
    private static String staticField = "외부 클래스의 정적 필드";
    ...
    static class StaticClass {
        void display() {
            // 외부 클래스의 인스턴스 변수에 접수 X
            // System.out.println("외부 클래스 필드에 접근: " + outerField);

            // 외부 클래스의 정적 변수 사용 시: 클래스명. 생략 가능
            System.out.println("외부 필드의 정적 필드에 접근: " + staticField);
        }
    }

    // 3. 메서드 내부 클래스(지역 클래스)
    void outerMethod() {
        final String localVar = "메서드 로컬 변수(Final)"; // 암묵적으로 final 간주 (생략)
...
public class A_Inner {
    public static void main(String[] args) {
        ...
        // 2) 외부 클래스.내부클래스 형식으로 내부 클래스를 생성
