...
public class A_sort {
    public static void main(String[] args) {
        ...
        // 문자열 길이 기준 정렬
        // 순회) name -> name.length, String::length ← (메서드 참조 방식)
        names.sort(Comparator.comparingInt(String::length));
        System.out.println(names);

        // == 람다식 사용 == //
        List<String> names2 = new ArrayList<>(List.of("Pororo", "Jerry", "Tom", "Ruffy", "C"));
        ...
        // - 두 요소를 비교한 결과를 반환
        //      1) 반환값이 양수 : b가 a 보다 앞에 옴
        //      2) 반환값이 0 : 순서 변경 X
        //      3) 반환값이 음수 : a가 b 보다 앞에 옴
        ...
        List<String> characterNames = new ArrayList<>(List.of("Pororo", "Jerry", "Tom", "Ruffy", "C"));
        List<String> sortedCharacter = characterNames.stream()
                .sorted((n1, n2) -> n1.length() - n2.length())
                .collect(Collectors.toList());

        // == Comparator 클래스 == //
        // : 사용자 정의 정렬 정의 시 유요한 인터페이스
        // - 람다식 & 메서드 참조와 함께 사용
